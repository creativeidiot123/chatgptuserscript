...
(() => {
  ...
  async function sendLiteralContinue(reason = 'incomplete') {
    ...
    maybeNotify(
      `${APP}: continue #${nextCount}/${CFG.maxContinuesPerLogicalTask}`,
      `Reason: ${String(reason || 'incomplete')}`,
    );
    return true;
  }
  ...
  function selfTest() {
    const cases = [
      ...
      ['bare connection interrupted tail still finds card', classifyRenderedTailError('normal assistant prose '.repeat(120) + ' Connection interrupted')?.id, 'connection-interrupted'],
